...
export const MetricCard = ({ title, value, suffix, delay = 0 }: { title: string, value: number, suffix: string, delay?: number }) => {
  const variants = {
    'PLATFORM ASSETS': {
      icon: Landmark,
      panel: 'from-[#64131a] via-[#3b0b10] to-[#170507]',
      glow: 'from-[#a62a35]/60 to-luxury-gold/35',
      watermark: 'text-[#d7a85b]',
    },
    'GLOBAL MEMBERS': {
      icon: Globe2,
      panel: 'from-[#2b1419] via-[#160a0d] to-[#080506]',
      glow: 'from-[#6f3542]/55 to-[#c19045]/30',
      watermark: 'text-[#c3a1a7]',
    },
    PROFIT: {
      icon: TrendingUp,
      panel: 'from-[#4a2410] via-[#2a1208] to-[#100704]',
      glow: 'from-[#a76022]/55 to-luxury-gold/45',
      watermark: 'text-luxury-gold',
    },
  } as const;
  const variant = variants[title as keyof typeof variants] ?? variants['PLATFORM ASSETS'];
  const MetricIcon = variant.icon;
  ...
  return (
    ...
    >
      <div className={`absolute -inset-1 rounded-2xl bg-gradient-to-r lg:-inset-2 lg:blur-3xl ${variant.glow} opacity-10 blur-xl transition duration-700 group-hover:opacity-35`}></div>

      <div className={`relative flex h-full min-h-[100px] flex-col items-center justify-center overflow-hidden rounded-2xl border border-luxury-gold/30 bg-gradient-to-br p-2 shadow-[0_15px_40px_-10px_rgba(0,0,0,0.8)] transition-all duration-500 group-hover:border-luxury-gold lg:min-h-[200px] lg:p-6 lg:shadow-[0_30px_70px_-15px_rgba(0,0,0,0.8)] ${variant.panel}`}>
        <div className="pointer-events-none absolute top-0 -inset-full z-20 block h-full w-3/4 -skew-x-12 bg-gradient-to-r from-transparent via-white/25 to-transparent opacity-0 group-hover:animate-shine"></div>
        <MetricIcon aria-hidden="true" className={`pointer-events-none absolute right-3 top-1/2 h-20 w-20 -translate-y-1/2 opacity-[0.08] transition-opacity duration-500 group-hover:opacity-[0.13] lg:right-8 lg:h-36 lg:w-36 ${variant.watermark}`} strokeWidth={1.1} />

        <div className="relative z-10 text-center">
          <div className="mb-1 inline-block lg:mb-4">
            <h4 className={`font-black uppercase text-white/90 drop-shadow-lg transition-colors duration-300 group-hover:text-luxury-gold-light ${title.length > 8 ? 'text-[6px] tracking-[0.12em] lg:text-[12px] lg:tracking-[0.3em]' : 'text-[7px] tracking-[0.2em] lg:text-[14px] lg:tracking-[0.6em]'}`}>
              {title}
            </h4>
            <div className="mt-0.5 h-0.5 w-full scale-x-50 bg-luxury-gold shadow-[0_0_10px_rgba(234,179,8,0.5)] transition-all duration-700 group-hover:scale-x-100 group-hover:bg-white lg:mt-2"></div>
          </div>
          <p className="text-3xl font-mono font-black tracking-tighter text-white drop-shadow-[0_4px_10px_rgba(0,0,0,0.6)] lg:text-7xl lg:drop-shadow-[0_8px_20px_rgba(0,0,0,0.6)]">
...
export const PhoneMockup = () => {
  ...
  return (
    <div className="relative animate-floating-phone" style={{ perspective: '2000px' }}>
      <div ref={frameRef} className="relative h-[706px] w-[336px] overflow-hidden rounded-[42px] border-[8px] border-[#292929] bg-black shadow-[0_50px_100px_rgba(0,0,0,0.9),0_0_0_1px_#555,inset_0_0_20px_rgba(255,255,255,0.1)]">
        {showMobileUi ? (
          <iframe
            title="LONGRISE Mobile UI preview"
            src="./?preview=mobile&v=unified-source-1"
            loading="lazy"
            tabIndex={-1}
            aria-hidden="true"
            className="pointer-events-none absolute left-0 top-0 border-0"
            style={{ width: '320px', height: '690px' }}
          />
        ) : (
          <div className="flex h-full items-center justify-center bg-[#090101]">
            <div className="text-center">
              <img src="./rank-logos/icon/gold.png" alt="" className="mx-auto h-12 w-12 object-contain opacity-70" />
              <p className="mt-3 font-serif text-sm font-black tracking-[0.16em] text-[#ead39b]">LONGRISE</p>
            </div>
          </div>
        )}
      </div>
...
export const HeroSection = ({ onStartClick, onAboutClick }: { onStartClick: () => void, onAboutClick: () => void }) => (
  <section className="relative min-h-[90vh] lg:min-h-screen flex items-center justify-center overflow-hidden pt-12 lg:pt-20">
    <div className="absolute inset-0 z-0">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,_rgba(72,0,0,0.22)_0%,_transparent_70%)]"></div>
    ...
    <div className="relative z-10 px-6 max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-3 gap-3 lg:gap-10 mt-12 lg:mt-16 pb-12">
        <MetricCard title="PLATFORM ASSETS" value={142} suffix="M+" delay={0.2} />
        <MetricCard title="GLOBAL MEMBERS" value={840} suffix="K+" delay={0.4} />
...
export const TechnologyStorySection = () => (
  <section id="tech-story" className="relative py-32 overflow-hidden">
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,_rgba(72,8,8,0.28)_0%,_transparent_70%)] opacity-30"></div>
