...
import { BookOpen, Zap, Users, HelpCircle, ChevronRight, FileText, Upload, Download, X, ChevronDown, Cpu, Rocket, TrendingUp, Shield, Layers, Database, Share2 } from 'lucide-react';
...
export const DocumentationPage = () => {
  ...
  const [whitepapers, setWhitepapers] = useState<{ id: string; name: string; url: string; uploadedAt: string }[]>([]);
  ...
  const handlePDFUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setWhitepapers([...whitepapers, newPaper]);
    e.target.value = '';
  };
        ...
        >
          ...
          {activeSection === 'whitepaper' && (
            <div className="space-y-8">
              ...
              {/* PDF Upload Area */}
              <div className="bg-white/5 border-2 border-dashed border-luxury-gold/50 rounded-xl p-8 text-center space-y-4">
                <div className="flex justify-center">
                  <Upload size={40} className="text-luxury-gold" />
                </div>
                <div>
                  <h3 className="text-xl font-black text-white mb-2">Upload LONGRISE Whitepaper</h3>
                  <p className="text-gray-400 mb-4">Upload PDF files for the LONGRISE whitepaper documentation</p>
              ...
              {whitepapers.length > 0 ? (
                <div className="space-y-4">
                  <h3 className="text-2xl font-black text-white">Available Documents</h3>
                  {whitepapers.map((paper) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: -10 }}
                      className="bg-white/5 border border-white/10 rounded-xl p-6 flex items-center justify-between hover:border-luxury-gold/30 transition-all"
                    >
                      <div className="flex items-center gap-4 flex-1">
                        <div className="p-3 bg-luxury-gold/20 rounded-lg">
                          <FileText size={24} className="text-luxury-gold" />
                        </div>
                        <div className="flex-1">
                          <h4 className="text-lg font-black text-white">{paper.name}</h4>
                          <p className="text-sm text-gray-400">Uploaded: {paper.uploadedAt}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <a
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-luxury-gold/20 border border-luxury-gold/50 text-luxury-gold rounded-lg hover:bg-luxury-gold/30 transition-all"
                        >
                          <Download size={18} />
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center">
                  ...
                  <p className="text-gray-400 text-lg">No whitepaper documents uploaded yet</p>
                </div>
              )}
            </div>
          )}
