...
export const USDTFraudReportPage = ({ onSetView }: { onSetView: (view: string) => void }) => {
  ...
  const handleAddEvidence = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmitReport = () => {
    if (!fraudsterUid.trim()) {
      alert('Please enter the fraudster UID');
      return;
    }
    if (!fraudReason) {
      alert('Please select a fraud reason');
      return;
    }
    if (!description.trim()) {
    ...
    const report: FraudReport = {
      id: `REPORT_${Date.now()}`,
      reporterUid: 'USER_12345', // Mock current user
      fraudsterUid,
