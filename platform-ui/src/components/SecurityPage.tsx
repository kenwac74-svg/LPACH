...
export const SecurityPage = ({ user, onUpdateUser }: { user: any, onUpdateUser: (u: any) => void }) => {
  ...
  const securityEvents = [
    ...
    { event: 'Security PIN Update', ip: '142.12.8.2', loc: 'Singapore', time: '2 days ago', status: 'Completed' },
  ];
          ...
          {protectionLayers.map((layer, i) => (
            ...
            >
              ...
              {layer.isSecure ? (
                ...
                <button className="px-5 py-2 rounded-lg bg-luxury-red-dark border border-white/5 text-gray-500 group-hover:text-luxury-gold group-hover:border-luxury-gold/50 text-[8px] font-black tracking-widest uppercase transition-all">
                  ACTIVATE
                </button>
              )}
            </motion.div>
